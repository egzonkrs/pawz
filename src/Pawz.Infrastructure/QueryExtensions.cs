...
public static class QueryExtensions
{
        ...
        where TEntity : Pet
    {
        ...
        // queryParams.TotalCount = await query.CountAsync(cancellationToken); // check if we can put it outside and pass it as parameter we cannot do async stuff here
        queryParams.TotalPages = (int)Math.Ceiling(queryParams.TotalCount / (double)queryParams.PageSize);  // check if we can put it outside and pass it as parameter
